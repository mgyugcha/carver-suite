...
function sorter (args) {
  const { emitter, inputdir, outputdir, io } = args
  return new Promise((resolve, reject, onCancel) => {
    recursive(inputdir, (err, files) => {
      ...
      Promise.map(files, file => {
        ...
        const chuck = i++ / length
        emitter.emit('chunck', chuck)
        io.emit('output-percent', chuck)
        return check_file(file, type, outputdir, statistics, io)
      }, { concurrency: 1 }).then(() => resolve(statistics)).catch(reject)
    })
  })
}

function check_file (file, type, outputdir, statistics, io) {
  return new Promise((resolve, reject) => {
    const ext = type ? type.ext : 'text'
    const good = () => {
      statistics.good++
      io.emit('output-file', `positivo: ${file}\n`)
      move('positivos')
    }
    const bad = () => {
      statistics.bad++
      io.emit('output-file', `falso-positivo: ${file}\n`)
    ...
    case 'webm':
      ...
      break
    default:
      io.emit('output-file', `sin clasificar: ${file}\n`)
      statistics.unknown++
      resolve()
      break
    }
